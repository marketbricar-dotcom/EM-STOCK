...
import { Plus, Edit2, Search, PackageOpen, Trash2, AlertCircle, FileDown, Sparkles, Calculator, ScanBarcode } from 'lucide-react';
...
const Inventory: React.FC<InventoryProps> = ({ products, setProducts, exchangeRate }) => {
  ...
  // Computed Products (Sorted & Filtered)
  ...
  const handleCostOrProfitChange = (cost: number, profit: number) => {
    ...
    // Auto-calculate price if both cost and profit are present
    if (cost > 0 && profit > 0) {
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-xl shadow-purple-100/50 border border-white sticky top-28">
            <div className="flex items-center gap-2 mb-6">
               ...
               <h3 className="text-lg font-bold text-slate-700">
                {isEditing ? 'Editar Cosita' : 'Nueva Cosita'}
              </h3>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-5">
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-4 pr-12 py-3 rounded-2xl bg-brand-bg border-2 border-transparent focus:border-brand-primary/50 focus:bg-white focus:ring-4 focus:ring-brand-primary/10 outline-none transition-all text-sm font-semibold text-slate-700 placeholder:text-slate-300"
                    placeholder="Escanea o escribe..."
                  />
                  <button 
                    type="button"
                    onClick={() => { setScanMode('form'); setShowScanner(true); }}
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-slate-400 hover:text-brand-primary hover:bg-white rounded-xl transition-all"
                  >
                    <ScanBarcode size={18} />
                  </button>
                </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-2xl bg-brand-bg border-2 border-transparent focus:border-brand-primary/50 focus:bg-white focus:ring-4 focus:ring-brand-primary/10 outline-none transition-all text-sm font-semibold text-slate-700 placeholder:text-slate-300"
                  placeholder="Ej. Blusa de Flores"
                />
              </div>

              {/* Cost & Profit Row - Optional */}
              <div className="grid grid-cols-2 gap-4 bg-brand-bg/30 p-3 rounded-2xl border border-dashed border-purple-200">
                <div>
                   <div className="flex items-center gap-1 mb-2">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Costo $ (Opc)</label>
                   </div>
                   <input
                    type="number"
                    min="0"
                    ...
                    placeholder="0.00"
                    className="w-full px-3 py-2 rounded-xl bg-white border border-transparent focus:border-brand-primary/30 outline-none text-sm font-bold text-slate-600 placeholder:text-slate-300"
                  />
                </div>
                <div>
                   <div className="flex items-center gap-1 mb-2">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Ganancia % (Opc)</label>
                   </div>
                   <div className="relative">
                     <input
                      type="number"
                      ...
                      placeholder="0"
                      className="w-full pl-3 pr-6 py-2 rounded-xl bg-white border border-transparent focus:border-brand-primary/30 outline-none text-sm font-bold text-slate-600 placeholder:text-slate-300"
                    />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-2xl bg-brand-bg border-2 border-transparent focus:border-brand-primary/50 focus:bg-white outline-none transition-all text-sm font-bold text-slate-700"
                  />
              ...
              <div>
                ...
                <div className="relative">
                    ...
                    className="w-full px-4 py-3 rounded-2xl bg-brand-bg border-2 border-transparent focus:border-brand-primary/50 focus:bg-white outline-none text-sm font-semibold text-slate-700 appearance-none cursor-pointer"
                    >
              ...
              <div className="flex gap-3 pt-4">
                <button
                  type="submit"
                  className="flex-1 bg-brand-primary hover:bg-brand-primaryHover text-white font-bold py-3.5 rounded-2xl transition-all hover:scale-[1.02] active:scale-95 shadow-lg shadow-purple-200 flex justify-center items-center gap-2 text-sm"
                >
                ...
                {isEditing && (
                  <button
                    ...
                    className="px-5 py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl transition-colors text-sm font-bold"
                  >
                    Cancelar
                  </button>
                )}
            ...
            <div className="p-6 bg-white border-b border-brand-bg flex flex-col sm:flex-row gap-4 items-center justify-between sticky top-0 z-10">
              <div className="flex items-center gap-2 w-full sm:max-w-md">
                 <div className="relative w-full group">
                    ...
                    className="w-full pl-12 pr-6 py-3 rounded-full bg-brand-bg border-2 border-transparent focus:bg-white focus:border-brand-primary/30 outline-none text-sm font-bold text-slate-700 transition-all placeholder:text-slate-300"
                    />
                 </div>
                 <button
                   onClick={() => { setScanMode('search'); setShowScanner(true); }}
                   className="p-3 bg-brand-dark/5 hover:bg-brand-primary/10 text-brand-primary rounded-full transition-colors flex-shrink-0"
              ...
              <button
                ...
                className="w-full sm:w-auto px-6 py-3 bg-brand-yellow/20 hover:bg-brand-yellow/40 text-brand-dark text-sm font-bold rounded-full transition-colors flex items-center justify-center gap-2"
              >
                 <FileDown size={18} />
                 <span>PDF</span>
              </button>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-brand-primary/10">
                    <th className="px-6 py-5 text-xs font-extrabold text-brand-primary uppercase tracking-wider rounded-tl-3xl">Producto</th>
                    ...
                    <th className="px-6 py-5 text-xs font-extrabold text-brand-primary uppercase tracking-wider text-center rounded-tr-3xl">Acciones</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-purple-50">
                  {filteredProducts.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="p-16 text-center text-brand-border">
                        ...
                        <td className="px-6 py-5 text-center">
                          <div className="flex items-center justify-center gap-2 opacity-60 group-hover:opacity-100 transition-all">
                            <button
                                ...
                                className="p-2 text-brand-secondary hover:text-brand-primary hover:bg-brand-primary/10 rounded-xl transition-colors"
                            >
                                <Edit2 size={18} />
                            </button>
                            <button
